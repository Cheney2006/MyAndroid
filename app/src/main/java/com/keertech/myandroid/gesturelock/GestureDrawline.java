...
public class GestureDrawline extends View {
    ...
    private static final int DEFAULT_LINE_COLOR = 0xFFE58E21;
    private static final int ERROR_LINE_COLOR = 0xFF9A0715;
    ...
    public GestureDrawline(Context context, List<GesturePoint> list, boolean isVerify,
        ...
        super(context);
        int displayWidth = AndroidUtil.getDisplayWidth(context);
        paint = new Paint(Paint.DITHER_FLAG);// 创建一个画笔
        bitmap = Bitmap.createBitmap(displayWidth, displayWidth, Bitmap.Config.ARGB_8888); // 设置位图的宽高
        canvas = new Canvas();
        canvas.setBitmap(bitmap);
        paint.setStyle(Style.STROKE);// 设置非填充
        paint.setStrokeWidth(10);// 笔宽5像素
        paint.setColor(DEFAULT_LINE_COLOR);// 设置默认连线颜色
    ...
    private void drawErrorPathTip() {
        canvas.drawColor(Color.TRANSPARENT, PorterDuff.Mode.CLEAR);
        paint.setColor(ERROR_LINE_COLOR);// 设置错误线路颜色
        for (Pair<GesturePoint, GesturePoint> pair : lineList) {
