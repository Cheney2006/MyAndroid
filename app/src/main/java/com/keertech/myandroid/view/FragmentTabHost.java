...
/**
 * *****************************************
 * Description ：带缓存的FragmentTabHost
 * 近要做一个类似QQ底部有气泡的功能
 * ，试了几个方案不太好，我想很多开发者使用TabHost都会知道它不保存状态，
 * 每次都要重新加载布局，为了保存状态，使用RadioGroup来实现，
 * 状态是可以保存了，问题是无法实现气泡功能，不能自定义布局，
 * 因为RadioGroup里面只能包含RadioButton，不然状态切换不起用作，
 * 这个可以查看RadioGroup源码，为了既能保存状态又能实现气泡功能，
 * 所以只能自己修改控件了或者自己写一个类似的切换功能，
 * 查看了FragmentTabHost的源码，
 * 可以知道FragmentTabHost不保存状态是因为切换fragment的时候是
 * 使用detach和attach来Fragment的隐藏和显示的，这样的话每次切换肯定要重新加载布局，
 * 处理使用detach和attach，我们还可以使用show和hide来实现显示和隐藏，
 * 这样可以保存状态，方案出来了就是修改FragmentTabHost源码将切换Fragment的方式
 * detach和attach改为hide和show。
 * Created by cy on 2014/11/13.
 * *****************************************
 */
